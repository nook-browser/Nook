...
struct HistoryRegression {
    static func main() async throws {
        ...
        let all = await store.history(days: 7, profile: nil, offset: 0, limit: 100)
        precondition(all.entries.count == 2, "Nil-profile adoption must not merge another profile")
        let adopted = await store.history(days: 7, profile: profile, offset: 0, limit: 1)
        ...
        let page = await store.search(query: "CAFÉ", profile: profile, offset: 50, limit: 50)
        ...
        let last = await store.search(query: "Café", profile: profile, offset: 550, limit: 50)
        precondition(last.entries.count == 50 && !last.hasMore)
        await store.clear(days: 0, profile: profile)
        let otherEntries = await store.history(days: 7, profile: other, offset: 0, limit: 1000)
        precondition(otherEntries.entries.count == 601, "Clearing one profile must preserve another")
        let stale = now.addingTimeInterval(-Double(HistoryManager.retentionDays + 1) * 86_400)
        await store.addVisits([HistoryVisit(url: URL(string: "https://stale.example")!, title: "Stale", timestamp: stale, tabId: nil, profileId: other)])
        ...
        let pruned = await store.history(days: HistoryManager.retentionDays * 2, profile: other, offset: 0, limit: 1000)
        precondition(!pruned.entries.contains { $0.title == "Stale" }, "The launch prune must reach every space, not only the launch one")
        ...
        print("PASS: index migration, profile isolation/adoption, batched writes, recency, localized paginated search, write ordering, incognito and retention across spaces")
    }
}
