...
class CacheManager: ObservableObject {
    ...
    func loadCacheData() async {
        ...
        cacheEntries = cacheInfos
        domainGroups = groupCacheByDomain(cacheInfos)
        isLoading = false
    }
    ...
    func clearStaleCache() async {
        ...
        let thirtyDaysAgo = Date().addingTimeInterval(-2_592_000) // 30 days
        let allTypes = WKWebsiteDataStore.allWebsiteDataTypes()
    ...
    func performPrivacyCompliantCleanup() async {
        ...
        let ninetyDaysAgo = Date().addingTimeInterval(-7_776_000) // 90 days
...
extension CacheManager {
    ...
    func getCacheEfficiencyRecommendations() -> [String] {
        ...
        if stats.totalSize > 1_073_741_824 { // 1GB
        ...
        if let largest = largeDomains.first, largest.totalSize > 104_857_600 { // 100MB
