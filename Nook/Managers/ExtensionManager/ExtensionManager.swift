...
final class ExtensionManager: NSObject, ObservableObject, WKWebExtensionControllerDelegate {
    ...
    var windowAdapter: ExtensionWindowAdapter?
    ...
    override private init() {
        context = Persistence.shared.container.mainContext
        isExtensionSupportAvailable = ExtensionUtils.isExtensionSupportAvailable
        super.init()
    ...
    private func configureMV3Extension(webExtension: WKWebExtension, context _: WKWebExtensionContext, manifest: [String: Any]) async throws {
    ...
    func uninstallExtension(_ extensionId: String) {
        ...
        do {
            ...
            let entities = try context.fetch(FetchDescriptor<ExtensionEntity>(predicate: predicate))
    ...
    private func loadInstalledExtensions() {
        do {
            let entities = try context.fetch(FetchDescriptor<ExtensionEntity>())
                                    ...
                                    let displayName = extensionContext.webExtension.displayName
                                {
                                    self.presentPermissionPrompt(
                                        ...
                                        onCancel: {
                                            ...
                                            for m in extensionContext.webExtension.requestedPermissionMatchPatterns {
                                                extensionContext.setPermissionStatus(.deniedExplicitly, for: m)
                                            }
                                        }
                                    )
    ...
    private var lastCachedAdapterLog: Date = .distantPast
    ...
    private var lastFocusedWindowCall: Date = .distantPast
    private var lastOpenWindowsCall: Date = .distantPast
    ...
    func webExtensionController(
           ...
           let resolvedContext = controller.extensionContext(for: url)
        {
            ...
            let tabAdapter = stableAdapter(for: newTab)
