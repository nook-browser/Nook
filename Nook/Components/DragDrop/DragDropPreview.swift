...
class MockTab: Identifiable, ObservableObject {
    ...
    var url: URL = .init(string: "https://example.com")!
...
class MockTabManager: ObservableObject {
    ...
    func handleDragOperation(_ operation: DragOperation) {
        ...
        case let (.spacePinned(spaceId), .spacePinned(toSpaceId)):
            if spaceId == toSpaceId {
                reorderSpacePinned(operation.tab as! MockTab, in: spaceId, to: operation.toIndex)
            }

        case let (.spaceRegular(spaceId), .spaceRegular(toSpaceId)):
            if spaceId == toSpaceId {
...
struct DragDropPreview: View {
    ...
    private func handleDrop(providers _: [NSItemProvider], toContainer: TabDragManager.DragContainer, atIndex: Int) -> Bool {
        #if DEBUG
