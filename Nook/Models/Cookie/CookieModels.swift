...
struct CookieInfo: Identifiable, Hashable, Codable {
    ...
    init(from httpCookie: HTTPCookie) {
        ...
        sameSitePolicy = httpCookie.sameSitePolicy?.rawValue ?? "None"
        size = (httpCookie.name.count + httpCookie.value.count)
    }
    ...
    var privacyRisk: PrivacyRisk {
        ...
        if let expiry = expiresDate, expiry.timeIntervalSinceNow > 31_536_000 { // > 1 year
            score += 1
        }
    ...
    var complianceIssues: [String] {
        ...
        if let expiry = expiresDate, expiry.timeIntervalSinceNow > 31_536_000 {
