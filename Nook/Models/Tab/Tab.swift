...
public class Tab: NSObject, Identifiable, ObservableObject, WKDownloadDelegate {
    ...
    enum LoadingState {
        ...
        var description: String {
            ...
            case let .didFailProvisionalNavigation(error):
    ...
    private var lastAudioDeviceCheckTime: Date = .init()
    ...
    var onCommandHover: ((String?) -> Void)?
    ...
    func loadURL(_ newURL: URL) {
        ...
        browserManager?.syncTabAcrossWindows(id)
    ...
    func requestPictureInPicture() {
        ...
        if let browserManager = browserManager,
           ...
           let activeWebView = browserManager.getWebView(for: id, in: activeWindowId)
        {
    ...
    private func injectMediaDetection(to webView: WKWebView) {
        ...
        webView.evaluateJavaScript(mediaDetectionScript) { _, error in
            if let error = error {
    ...
    private func injectPiPStateListener(to webView: WKWebView) {
        ...
        webView.evaluateJavaScript(pipStateScript) { _, error in
            if let error = error {
    ...
    func updateTitle(_ title: String) {
        name = title.isEmpty ? url.host ?? "New Tab" : title
        if #available(macOS 15.5, *) {
...
extension Tab: WKNavigationDelegate {
    ...
    public func download(_: WKDownload, decideDestinationUsing _: URLResponse, suggestedFilename: String, completionHandler: @escaping (URL, Bool) -> Void) {
