...
**There is no test target.** `xcodebuild test -scheme Nook` fails. Verification is a build plus manual run. Pure logic that deserves a check can keep one without a test target by compiling the shipped source directly, the way `Packages/NookWeb/Checks/AutofillRankingCheck.swift` does: an `@main` struct of `precondition`s, run with `swiftc <source> <check> -o /tmp/x && /tmp/x`. A `Checks/` directory beside `Sources/` is outside the SPM target path, so nothing else picks it up. If the working tree holds someone else's uncommitted edits, verify committed work in a detached worktree (`git worktree add --detach /tmp/x HEAD`) with its own `-derivedDataPath` instead of building the dirty tree.
...
- **Releases are tags.** A push of `vX.Y.Z-beta.N` runs the notarize workflow as a beta: the app's marketing version becomes `X.Y.Z-beta.N`, the GitHub release is a prerelease titled with the tag and a beta notice at the top of its notes, and the appcast item and DMG volume are titled "Nook X.Y.Z beta N". A push of `vX.Y.Z` is the official release. Every tag needs its own `CURRENT_PROJECT_VERSION` bump first, since Sparkle orders by build number (1.1.0-beta.1 went out as build 111; the 110 run failed). Release notes are the block under the version's `## ` heading in `CHANGELOG.md`; the workflow copies it into the release body and falls back to GitHub's generated notes when the heading is missing. Betas and official releases share the default Sparkle channel for now; a beta channel behind a Settings toggle comes with the first official release under the new bundle id.
...
| **SocialImageTweaks** | Download button on photos and videos on any site the user lists in Tweaks > Social Media (`settings.mediaDownloadSites`). The 1.1.0-beta.1 per-site toggles were dropped without seeding this list, accepted once for beta users; from 1.1.0 on, a persisted settings key is never removed or renamed without reading the old value into its replacement. Installed on the first visit to a listed site and rebuilt only when the list changes; the script gets the domains as `NOOK_DOWNLOAD_SITES` and suffix-matches the hostname. `social-image-download.js` (isolated world) finds media under the pointer, taking any https image or video, since a listed site may serve from any CDN; `social-video-source.js` (page world) reads MP4 URLs from React data because Facebook and Instagram play blob: URLs. Saves through the `MediaDownloading` seam (`FocusableWKWebView.downloadImage` on the app side). Lives in `Packages/NookTweaks/Sources/NookTweaks/`. |
| **TabOrganizerManager/** | Tab grouping, renaming and duplicate closing on the system language model (`FoundationModels`, Apple Intelligence). `TabOrganizationModel` groups the loose tabs on their own: one turn names folders, a second files each tab by writing its topic and then choosing a folder name or "Unrelated" (`DynamicGenerationSchema` `anyOf`, one entry per tab), so there is no output parsing. Existing folders are matched afterwards; offered during grouping they used up the folder budget and pulled unrelated tabs in. A new group joins an existing folder when one name's words contain the other's (`sameTopic`, since most folders were named by an earlier run), and only the rest go to a strict model match: asked loosely, the model handed every new group to some existing folder. A single loose tab never joins an existing folder: folders as filing choices, a leftover pass, a yes/no check on each pick, `NLEmbedding` distance and regrouping beside the folders' tabs were all tried, and each either misfiled tabs or joined nothing. Proposed folders with a generic name (`genericNames`: Technology, Personal, Misc) are dropped. Every generated array has a pinned length and every call a response-token cap: an unbounded array once ran to the 8192-token context. Sampling is `.greedy`; `temperature: 0` alone varied between runs. Stay off `FoundationModels` symbols newer than the 26.0 SDK (`contextSize` needs 26.4, `GenerationOptions(samplingMode:)` needs 27): back-deployment covers the OS, not the SDK, and CI does not pin Xcode. Renames are a separate concurrent call, and front pages take the site's name from the title in code. Duplicates are a URL comparison that includes tabs already in folders. Pinning a tab shortens its title when Settings > AI > Rename Pinned Tabs is on (`TabsController.onTabsPinned`). When `SystemLanguageModel` is unavailable the sidebar button and menu item are hidden and Settings > AI shows why. The separator (`SpaceSeparator`) ripples while a run is in progress. Stays in `Nook/Managers/TabOrganizerManager/`. |
| **DialogManager/** | Per-window stacks for in-window dialogs: space creation, basic auth, Settings, import, confirmations |
...
| **PeekManager/** | Peek: a link previewed in a card over one window, as a detached `PageSession` (see Key Patterns). Opens on its own only from a favorite or pinned tab to another site, or on Option-click anywhere. The close/split/new-tab buttons are a `nookGlassControls` capsule left of the card. |
...
| **ExternalMiniWindowManager** | Mini browser windows for a link from another app and for a sign-in popup (a real WebKit popup, so `window.opener` survives and `window.close()` closes the window). Pages are detached `PageSession`s. `OAuthDetector.knownProviderHosts` lists sign-in-only hosts, never whole sites: facebook.com there once sent every Facebook outbound link to this window (`Packages/NookBlocker/Checks/OAuthDetectorCheck.swift`). |

### State Management

- **`@Observable`** (Swift Observation): `TabsController`, `PageSession`, `Profile`, `BrowserWindowState`, `WebViewCoordinator`, `WindowRegistry`, `AIService`, `PeekManager`
- **`@Published` / `ObservableObject`** (Combine): `BrowserManager`, `ExtensionManager`, `NookDragSessionManager`
...
| `Nook/Components/` | SwiftUI views. `Settings/` holds the in-window `SettingsWindow` panel and the `SettingsTabs` enum in `SettingsUtils.swift` (12 tabs: general, appearance, ai, privacy, adBlocker, airTrafficControl, spaces, shortcuts, extensions, advanced, plus youTube and socialMedia under the sidebar's "Tweaks" section, which holds per-site tweaks and SponsorBlock). `Packages/NookUI/Presentation/NookPanel.swift` owns the reusable panel surface and overlay layout. |
...
- **Glass only on layers that float over content**: command palette, toasts, hover sidebar overlay, find bar, dialog cards, extension panels, split drop card, zoom popup, update notification. `nookGlassEffect(in:)` in `Packages/NookDesign/Sources/NookDesign/View+GlassEffect.swift` is the only entry point and already includes `.floating` elevation. The sidebar itself is `BlurEffectView(material: .sidebar)` in `WindowView`, and rows are `Surface` fills. The sidebar's selection and chrome controls are the exceptions, so the interface reads as Liquid Glass: the selected row, split row and pinned tile through `nookRowSelection(_:radius:)`, and the controls through `nookControlGlass(_:in:)` (the back/forward/reload pill in `SidebarHistoryButtons`, the sidebar/AI pill, the two bottom bar circles, and in the history and downloads panel the search fields, the chosen filter chip, the back button and the recent-downloads hover card; the chosen History/Downloads tab uses `nookRowSelection`). Chrome controls are `Size.glassControl` (36pt, a macOS 26 toolbar button) through `nookGlassControls(in:)`. Pills and circles, not squircles: Bain tried `Radius.md` on the history group and went back to Apple's pill. The AI chat panel follows the same rules: its header controls are `nookGlassControls`, the compose box and message bubbles are `nookControlGlass` (the person's own messages tinted with the space accent, neutral in a private window), and its text uses the roles, not a colour derived from the accent. A selected tile drops its fill, since a fill in front of glass flattens it. Never glass on glass: inside the hover overlay (`\.nookInsideGlass`) all of them fall back to a raised fill.
- **A floating layer must live in the window, never in an `NSPanel`.** Only the key window renders the active Liquid Glass appearance; a non-key panel always gets the inactive, foggy one, and nothing opts out of that (`orderFrontRegardless`, `.nonactivatingPanel`, `canBecomeKey` were all tried). Making a panel key is not an escape either: macOS gives a key window a heavier shadow, which on a borderless transparent panel squares off at the window rect and reads as a hard box around the rounded glass. The extension library and its overflow menu were ported out of panels in September 2026 for exactly this; `ExtensionLibraryOverlay` is the pattern, using an anchor preference from the button and `.overlayPreferenceValue` in `WindowView`, with a tap target plus `onExitCommand` for dismissal instead of an `NSEvent` monitor.
- **Never put a fill in front of glass, and never assume glass is dark.** `nookGlassEffect` is untinted `.regular` glass, because sampling the page behind it is the whole effect; a `.background(...)` applied before it sits on top and flattens that into a plain blur, and a tint inside it mutes it in the same direction more subtly. Contrast is the content's job, not the surface's. Four of nine floating layers did that until September 2026, as a leftover from the pre-macOS-26 fallback, and the fill was quietly supplying contrast: Liquid Glass takes its brightness from the page behind it while `.primary` follows the color scheme, so a pale page under a dark-mode window leaves light text on a light card. That is one bug with two faces. Fixed light text (toasts, palette rows, sidebar menu) vanished in light mode; fixed dark text (update notification) vanished in dark. Text is `.primary` / `.secondary` / `.tertiary` and fills are `Surface.fill` / `fillPressed`; a fixed `.white` is only correct over a fixed backdrop (the mini window's blue gradient, media artwork) or as a `Garnish.contrastingShade` fallback.
...
- **Space editing lives inline in Settings.** Renaming a space or changing its accent happens in Settings > Spaces (`SpacesSettingsView` in `Packages/NookUI/Sources/NookUI/Settings/Spaces.swift`): a `TextField` writes through `TabsController.updateSpace` on every keystroke, and an accent swatch opens `SpaceAccentPicker` in a popover. Every "Space Settings" entry point calls `BrowserManager.openSettings(tab: .spaces)`, which presents Settings in the active browser window. `DialogManager` keeps panels stacked per window, so a child dialog can appear above Settings and closing it restores the same Settings view. `SpaceCreationDialog` (New Space…) asks for name and accent; `SpaceDeleteConfirmationDialog` handles destructive confirmation.
...
- **Trails**: a tab opened from a tab's page (Cmd-click, middle-click, Open Link in New Tab, a popup that becomes a tab, Peek's Move to Tab) is that tab's last child, stored as `.folder(itemID: parentTab)`, so the file format did not change and an older build flattens trails on load. Trails count toward depth 5 (past it, `childTabPosition` falls back to a sibling), and a parent row draws the folder chevron (`Row.hasChildren`, open state in `openFolders`). Closing a parent moves its children up into its place (`close(_:promotingChildren:)`); `remove`, Clear, Close Other Tabs and Close All Below take whole trails. Moving into pinned or favorites flattens every trail in the moved subtree. Other new tabs open right below the selected tab (`newTabPosition`); a caller that names a parent keeps the top of it. The iCloud sync branch assumes only folders have children and must learn this before it merges.
...
- **`AppleIntelligenceProvider`** is the on-device chat on `SystemLanguageModel`, the model the tab organizer uses. `AIConfigService` lists it first and makes it active when no provider is usable (no provider, no model, or a missing key); without Apple Intelligence it is hidden. It gets its own short instructions, a page, history and tool-output budget in estimated tokens (4096-token context on macOS 26, 8192 on 27), and eight browser tools, not the page readers or JavaScript. The framework runs tools inside one response, so they reach `AIService.runOnDeviceTool` through a closure and keep the usual approval and switches; the provider never returns `.toolCalls`. On overflow it retries once without history, unless a tool already ran. The model followed instructions planted in page text on every eval run whatever the prompt said, so its opening and clicking tools ask for approval, even in Auto mode, unless the person's own message names the site or the button (`personNamed`); a click by selector always asks. It never searched past a cut-off page until told the text stops early, so `extractPageContext` adds that note outside the untrusted tag, and it never called `getSelectedText`, so the selection rides in the page context as `<selected_text>` (both for every provider). Scored eval of 22 cases: `docs/superpowers/on-device-chat-eval/`; run it before changing the instructions. **macOS 27 throws the 27 SDK's `LanguageModelError`, not `GenerationError`**, and CI's Xcode may lack that type, so errors are matched by case name through `Mirror`; a `catch` on `GenerationError` alone misses every error on 27.
- **The default system prompt migrates by hash.** Everyone who never edited it has the old text saved as their own, so `AIConfigService.replaceUneditedSystemPrompt` swaps a saved prompt whose SHA-256 matches the previous default. Put the outgoing default's hash there whenever the default changes.
...
- **AdvancedRulesEngine** + **BlockerEngine**: per-URL cosmetic lookup via adblock-rust's `Engine::url_cosmetic_resources`. `BlockerEngine` owns the one `adblock::Engine` in the app and is `@MainActor` with a synchronous lookup, because the main-frame config script must be installed before navigation commits; only `build` steps off-actor. Plain cosmetic filters never reach it: the converter turns them into `css-display-none` entries WebKit applies itself. **Scriptlets run from uBlock Origin's library**, vendored verbatim under `Packages/NookBlocker/ThirdParty/ubo-scriptlets/` at a pinned commit and turned into `Resources/scriptlets.json` by `scripts/build-scriptlets.mjs`; refresh the pin with `scripts/refresh-ubo-scriptlets.sh` and regenerate. 180 resources, 127 injectable, covering every `##+js()` name the bundled lists use. Two upstream directories are vendored: `src/js/resources/` for the scriptlets, and the `data: 'text'` entries of `src/web_accessible_resources/` for uBO's redirect resources, which double as argument-less scriptlets (`##+js(noeval)`). **Those bodies are GPL-3.0 and are not Nook's**, so the macOS build links them and ships as a notarized DMG, and `ScriptletResources.json()` returns nil on iOS. That `#if os(iOS)` is a licence boundary, not a feature flag. The five uBO lists plus `nook-filters-default` are marked `isTrusted`, which is what enables the `trusted-*` scriptlets including uBO's current YouTube technique. A rule naming a resource that does not exist resolves to nothing with no log at all (`resource_storage.rs` drops the `Err`), so verify with `Nook/ThirdParty/AdblockRustFFI/tests/real_lists_scriptlets.rs` rather than assuming. True procedural operators (`:has-text`, `:upward`, `:matches-css`) are skipped.
...
- **Resources/*-blocker.js**: site-specific scripts (Facebook, Instagram, X), static, main frame only, hostname-guarded. The YouTube one was deleted in September 2026: uBO's scriptlets cover its object traps, its cosmetic rules cover the DOM cleanup, and its seek-to-end fallback was what made an ad visible before it was skipped.
- **Facebook streams ad-blocker canaries; do not prune them.** ~4KB payloads holding three `__typename: "SponsoredData"` stubs at `data.node`, `edges[]` and `require[]`, a rotating single-key wrapper (`q7z`, `0`, …), a `p` field padded with one repeating 20-hex block, and no ad content whatsoever. On a measured feed scroll 34 payloads carried a sponsored marker, 9 carried a feed connection name, and **none carried both**, so a low match rate against `facebook-feed-prune.js`'s `WANT` gate is the gate working, not failing. Widening it to match `SponsoredData` removes nothing and hands Facebook the signal it is fishing for. A real ad arrives under a connection the `CONNECTIONS` list names.
...
- **Resources/nook-stealth-redirects.js**: answers a blocked ad URL with the resource uBO's `$redirect=` rules name for it (469 rules, generated into `redirects.json` by `scripts/build-redirects.mjs`, bodies from the vendored web-accessible resources), so a blocked request does not read as a failure to anti-adblock scripts and a page that needs `googletag` or `google.ima` gets a working shim. Covers what JavaScript starts (`fetch`, `XMLHttpRequest`, `src` on script and image elements built in code); a `<script src>` in the page's own HTML cannot be covered, since WebKit starts that load as the parser reaches the tag. The table is per navigation and main frame only, carried in the same config script as the cosmetic rules. **`$redirect-rule=` is excluded**: it applies only when the request would be blocked anyway, and `*$script,redirect-rule=noopjs` compiles to `.*`, which would empty every script on every page. A literal-length floor rejects any other pattern that loose, and `scripts/check-redirects.mjs` guards both.
...
- **`if-domain` needs a `*` prefix to reach subdomains.** WebKit matches an `if-domain`/`unless-domain` entry exactly unless it starts with `*`. adblock-rust prefixes network rule domains (`$domain=`) but not cosmetic hostnames, so `reddit.com##shreddit-ad-post` came out as `if-domain: ["reddit.com"]` and never fired on `www.reddit.com`: every site-scoped cosmetic filter was dead on any host with a subdomain. `content_blocking_ffi::widen_domains_to_subdomains` adds the prefix to every converted rule. The cache hash is over the input rules, so a converter change does not invalidate it on its own; bump `converterStampFile`'s name when output changes for the same input.
...
- **`cargo` under `Nook/ThirdParty/AdblockRustFFI` breaks the Xcode build.** It writes `target/`, which sits inside a filesystem-synchronized group, so every Rust build artifact is copied into the app bundle and the build dies with a wall of "Multiple commands produce .../Contents/Resources/build-script-build". `.gitignore` covers the directory but Xcode does not read it. Delete `target/` after running the Rust tests.
...
- **Detached pages**: Peek and mini window pages are full `PageSession`s outside the tree (`TabsController.openDetached`, `openDetachedPopup`), so they get the blocker, tweaks, downloads, dialogs and crash recovery that tabs get. `detachedProfile` and `detachedWindow` stand in for the item's space and window; `adopt(_:in:from:)` inserts an item for the live session and `endDetached` tears it down. Extensions cannot see a detached page until it is adopted. `DetachedPageHost` shows one.
- **Never resize a WKWebView before it is in a window.** WebKit resizes its inner drawing view (`WKFlippedView`) for the first off-window frame change only, and a SwiftUI representable is sized off-window (first 2000x1500, then its real size). The page then drew at 2000x1500 offset inside a smaller Peek card, though `frame`, `innerWidth` and zoom all read correctly. `DetachedPageHost` attaches the view in `viewDidMoveToWindow`. To diagnose a page that draws in the wrong place, log the web view's subviews' frames, not the web view's.
...
- **A page-world message handler is callable by every site and every ad iframe.** Anything that acts on a message (opens a tab, starts a download, changes what the URL bar shows, talks to the network) registers in an isolated `WKContentWorld` and checks `message.frameInfo` in Swift; `WebStoreScriptHandler` and `WebContextMenuBridge` are the shape. `PageSession`'s remaining page-world handlers drop subframe messages at the top of `userContentController(_:didReceive:)`, and none of them trusts a URL from the message body: `historyStateDidChange` reads `message.webView?.url`. Do not bring back a `chrome.identity`-style handler for pages; the old `NookIdentity` one returned OAuth redirect URLs to any site that asked.
...
- **Nook draws the PDF viewer's controls, not WebKit.** `BrowserConfiguration.hidePDFHUD` turns WebKit's bar off through the `PDFPluginHUDEnabled` feature flag, and `PDFControlsView` floats a glass pill over any page where `PageSession.isDisplayingPDF` (`_isDisplayingPDF`; KVC on `_displayingPDF` throws). Zoom goes through `ZoomManager` like Cmd+/-; Save and Open read the original bytes with `_getMainResourceDataWithCompletionHandler:`, and Save lands in the downloads list through `DownloadManager.saveCompletedFile`. WebKit's own Save button needs `_webView:saveDataToFile:suggestedFilename:mimeType:originatingURL:` or it does nothing and logs nothing; that method stays as the fallback for a WebKit that ignores the flag. The flag is global, so a PDF in an `<embed>` or iframe has no bar at all. The controls show on every PDF load and come back as the pointer moves over the page (`PageSession.pdfControlsReveal`, bumped by `pdfLoadStateChanged` and by `FocusableWKWebView.mouseMoved` through `pointerMovedOverPage`, four publishes a second at most, PDFs only).
- **One print path.** Cmd+P (`ShortcutAction.printPage`), File > Print…, the PDF pill's Print button and a page's own `window.print()` (`_webView:printFrame:pdfFirstPageSize:completionHandler:`, which WebKit silently drops without it) all go through `WKWebView.nookPrint` in `Packages/NookWeb/Sources/NookWeb/WKWebView+Print+macOS.swift`. `printOperation(with:)` plus `runModal(for:)` prints a web page and every page of a PDF; measured on macOS 27 by printing to a file and reading the text back, so no PDFKit route or `view.frame` workaround is needed. A subframe's print() prints that frame through `_printOperationWithPrintInfo:forFrame:`. The page waits on the completion handler until the sheet closes.
- **SwiftUI leaves web views out of the title band's drag region.** AppKit asks the content view which parts of the title band must not move the window (`_regionForOpaqueDescendants:forMove:forUnderTitlebar:`). SwiftUI's hosting view answers with an empty region and ignores the AppKit views inside it, so with the unified toolbar the whole 66pt band over the page dragged the window and text there could not be selected. `TitlebarDragRegion` (App/Window), called from `BackgroundWindowModifier` for every browser window, makes that one answer come from NSView's own implementation, which counts the web view. Hit testing was never the problem: the web view got the click either way. Measure with `_regionForOpaqueViewsBlockingDraggableFrame:` on the window rather than a hit test. It runs a handful of times a minute, under a millisecond in total. If SwiftUI changes the method's signature the patch skips itself and the band drags as before.
- **New default shortcuts need `currentVersion` bumped** in `KeyboardShortcutManager`. Saved shortcuts merge new defaults only when the stored version is lower, so a new action without the bump never reaches existing users; the merge also skips a default whose key the user gave another action.
- **The traffic lights have one owner**: `TrafficLightsView` in `App/Window/WindowView.swift` is the only code that sets `standardWindowButton(_:).isHidden`. It shows them with the sidebar or the hover sidebar and hides them across a full-screen exit. A second writer fights it: the full-screen code once restored the state it read at `willExitFullScreen`, which AppKit has already set to hidden, and the lights never came back.
...
| **Garnish** | Garnish | Color contrast/mixing utilities. Pinned by revision, see below | DialogManager, CommandPaletteView, NookButtonStyle |
...
Nook calls one symbol from it, `Garnish.contrastingShade`, at three call sites.
...
**Nook is GPL-3.0 throughout.** It links uBlock Origin's GPL-3.0 scriptlet library (see Content Blocker System); AdGuard's SafariConverterLib was replaced by `adblock-rust` under MPL-2.0 in September 2026.
