...
- **Glass only on layers that float over content**: command palette, toasts, hover sidebar overlay, find bar, dialog cards, extension panels, split drop card, zoom popup, update notification. `nookGlassEffect(in:)` in `Packages/NookDesign/Sources/NookDesign/View+GlassEffect.swift` is the only entry point and already includes `.floating` elevation. The sidebar itself is `BlurEffectView(material: .sidebar)` in `WindowView`, and rows are `Surface` fills, with two exceptions: the selected row (and the split row) through `nookRowSelection(_:)`, and the back/forward/reload group in the title row (`SidebarHistoryButtons`, Safari's look) through `nookControlGlass(in:)`. Never glass on glass: inside the hover overlay (`\.nookInsideGlass`) both fall back to a raised fill.
